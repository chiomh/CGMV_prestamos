...
!
  ...
  if ( ftol < 0.0D+00 .or. xtol < 0.0D+00 .or. gtol < 0.0D+00 &
...
!
    if ( ratio <= 0.25D+00 ) then
      ...
      if ( fnorm <= 0.1D+00 * fnorm1 .or. temp < 0.1D+00 ) then
        temp = 0.1D+00
      end if
      ...
      if ( par == 0.0D+00 .or. 0.75D+00 <= ratio ) then
        delta = 2.0D+00 * pnorm
        par = 0.5D+00 * par
      end if
...
!    Output, integer NFEV, the number of calls to FCN.
!
!    Output, real ( kind = 8 ) FJAC(LDFJAC,N), an M by N array.  The upper N by N submatrix
...
!
!    Output, real ( kind = 8 ) X(N), the least squares solution of the system A*X = B, 
!    sqrt(PAR)*D*X = 0, for the output value of PAR.
!
!    Output, real ( kind = 8 ) SDIAG(N), the diagonal elements of the upper triangular 
!    matrix S.
!
  implicit none
...
!
        if ( sdiag(k) /= 0.0D+00 ) then
          ...
          if ( abs ( r(k,k) ) < abs ( sdiag(k) ) ) then
            ...
            t = sdiag(k) / r(k,k)
            c = 0.5D+00 / sqrt ( 0.25D+00 + 0.25D+00 * t * t )
            s = c * t
          end if
!
...
!
    ...
    if ( rowj /= 0.0D+00 ) then
      ...
      if ( abs ( r(j,j) ) < abs ( rowj ) ) then
        ...
        t = rowj / r(j,j)
        c(j) = 0.5D+00 / sqrt ( 0.25D+00 + 0.25D+00 * t**2 )
        s(j) = c(j) * t
      end if
!
